...
| `set_component_property` | Set property on SCS or inherited component. Inherited components go through the child BP's override template; `AttachSocketName` on a locally-authored scene component also updates the runtime SCS attachment metadata. Pass value=null to clear an object reference (#420). Params: `assetPath, componentName, propertyName, value` |

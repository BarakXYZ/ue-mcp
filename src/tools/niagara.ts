...
export const niagaraTool: ToolDef = categoryTool(
  ...
  "Niagara VFX: systems, emitters, spawning, parameters, and graph authoring.",
  {
    ...
    list_module_inputs:  bp("List actual stack modules + input pins, stable node GUIDs, and enabled state for an emitter. Params: systemPath, emitterName?, emitterIndex?, moduleName?, stackContext? (ParticleSpawn|ParticleUpdate|EmitterSpawn|EmitterUpdate|all — default all)", "list_niagara_module_inputs"),
    ...
    set_module_enabled:  bp("Enable or disable exactly one Niagara stack module and force a GPU-aware recompile. Prefer nodeGuid from list_module_inputs; moduleName is accepted only when unambiguous. Params: systemPath, enabled, nodeGuid? | moduleName?, emitterName?, emitterIndex?, stackContext?", "set_niagara_module_enabled"),
  ...
  undefined,
  {
    ...
    nodeGuid: z.string().optional().describe("Stable Niagara function-call node GUID returned by list_module_inputs"),
    enabled: z.boolean().optional().describe("For set_module_enabled: requested module enabled state"),

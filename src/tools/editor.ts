...
export const editorTool: ToolDef = categoryTool(
  ...
  "Editor commands, Python execution, PIE, undo/redo, hot reload, viewport, performance, sequencer, build pipeline, logs, editor control.",
  {
    ...
    play_in_editor: bp("PIE/Standalone control. Params: pieAction (start|stop|status), playMode? (PlayMode_InEditorFloating|PlayMode_InNewProcess; omit to use Unreal's default FRequestPlaySessionParams path. PlayMode_InEditorFloating is in-process PIE in a floating window; PlayMode_InNewProcess launches the real Standalone Game new-process path), waitForAssetRegistry? (start only; default true - block until the AssetRegistry initial scan completes before requesting play, otherwise the request can silently no-op on cold editor starts), assetRegistryTimeoutSeconds? (default 180) (#406)", "pie_control", (p) => ({ action: p.pieAction ?? "status", playMode: p.playMode, waitForAssetRegistry: p.waitForAssetRegistry, assetRegistryTimeoutSeconds: p.assetRegistryTimeoutSeconds })),
  ...
  undefined,
  {
    ...
    playMode: z.enum(["PlayMode_InEditorFloating", "PlayMode_InNewProcess"]).optional().describe("play_in_editor start: Unreal EPlayModeType-style selector. PlayMode_InEditorFloating is in-process PIE in a floating window; PlayMode_InNewProcess launches real Standalone Game in a new process. Omit to use Unreal's default FRequestPlaySessionParams path."),
